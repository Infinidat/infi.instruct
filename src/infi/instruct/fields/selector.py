from ..base import AllocatingReader, Writer, ReprCapable, Sizer, ApproxSizer, is_sizer, is_approx_sizer
...
class StructSelectorIO(AllocatingReader, Writer, ReprCapable):
    def __init__(self, key_io, mapping, default=None):
        ...
        structs = mapping.values() + ([ default._io_ ] if default is not None else [])
    ...
    def create_from_stream(self, stream, context=EMPTY_CONTEXT, *args, **kwargs):
        ...
        if key not in self.mapping and default is None:
