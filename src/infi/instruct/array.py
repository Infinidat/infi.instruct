...
class FixedSizeArrayIO(AllocatingReader, Writer, ReprCapable):
    def __init__(self, size, element_io):
        ...
        install_mixin_if(self, Sizer, is_sizer(self.element_io))
        install_mixin_if(self, ApproxSizer, is_approx_sizer(self.element_io))
...
class SumSizeArrayIO(AllocatingReader, Writer, ReprCapable):
    def __init__(self, size_io, element_io):
        ...
        install_mixin_if(self, Sizer, is_sizer(self.element_io) and is_sizer(self.size_io))
    ...
    def _ApproxSizer_min_max_sizeof(self, context=EMPTY_CONTEXT):
        ...
        # the max element size and get a number that's less than maxint. But for our purposes it's enough to do this:
